...
conn_test = MySQLdb.connect(host="127.0.0.1", user="root", passwd="",
                            db="yuyicloud_test", charset="utf8")
...
##################################
# print "CommandLine is \"%s\" hashcode is \"%s\" "
# %(commandLine,hashcodeNu)
...
#                    #连接到对应的数据库
                    sql_insert_data = "INSERT INTO parse_match_keyword_" + \
                        tbNu + " VALUES " + desc_values
                    dbInsert = conn_nu.cursor()
                    outthing = dbInsert.executemany(
                        sql_insert_data, get_select_parameter_datas)
